...
// Base URL for the backend API. Set VITE_API_BASE_URL in your environment
// (e.g. a Vercel project env var) to point at your deployed backend.
// Falls back to localhost for local development so nothing breaks if unset.
...
export default function App() {
  ...
  const fetchChats = async (userId: string) => {
    try {
      const r = await fetch(`${API_BASE}/chat/users/${userId}/chats`)
      if (r.ok) {
  ...
  const handleLogin = async () => {
    ...
    try {
      const r = await fetch(`${API_BASE}/chat/login?username=${encodeURIComponent(usernameInput)}&password=${encodeURIComponent(passwordInput)}`, {
        method: "POST"
      })
      const d = await r.json()
      if (!r.ok) {
  ...
  const handleRegister = async () => {
    ...
    try {
      const r = await fetch(`${API_BASE}/chat/register?username=${encodeURIComponent(usernameInput)}&password=${encodeURIComponent(passwordInput)}`, {
        method: "POST"
      })
      const d = await r.json()
      if (!r.ok) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      setMessages((prev) => [
        ...prev,
        { id: `sys-${Date.now()}`, role: 'system', text: '❌ Cannot reach server. Is the backend reachable?' }
      ])
    } finally {
      setIsSending(false)
    }
